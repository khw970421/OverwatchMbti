    ...
    if ((endTime.getTime() - startTime.getTime()) / (1000) < 20) {
      ...
      if (isMore) {
        ...
        for (let i = 0; i < result[mbti]["line"].length; i++) {
          document.getElementById('line').insertAdjacentHTML('afterend', `<h3 class="text-center mt-1 skyblueText properSize">${result[mbti]["line"][i]}</h3>`);
        }
      } else {
        ...
        for (let i = 0; i < result2[mbti]["line"].length; i++) {
          document.getElementById('line').insertAdjacentHTML('afterend', `<h3 class="text-center mt-1 skyblueText properSize">${result2[mbti]["line"][i]}</h3>`);
        }
      }
    }
  }
